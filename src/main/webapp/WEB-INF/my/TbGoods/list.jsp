...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>my/tbgoods/query.html">
						...
						<c:when  test="${(list)!= null && fn:length(list) > 0}">
							 <c:forEach var="d" items="${list}" varStatus="status">
									...
									<td><span data-toggle="tooltip"   title="${(page.page-1)*page.pageSize+status.index + 1}">${(page.page-1)*page.pageSize+status.index + 1}</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsNumber }">${d.goodsNumber }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsName }">${d.goodsName }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsPrice }">${d.goodsPrice }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsDescribe }">${d.goodsDescribe }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsCreateTime }">${d.goodsCreateTime }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsCreateUser }">${d.goodsCreateUser }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsPostage }">${d.goodsPostage }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsScprice }">${d.goodsScprice }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsKccount }">${d.goodsKccount }</span></td>
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/my/TbGoods/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>my/tbgoods/query.html?pageNo={{number}}',
