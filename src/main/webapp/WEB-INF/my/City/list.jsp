...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>my/city/query.html">
						...
						<c:when  test="${(list)!= null && fn:length(list) > 0}">
							 <c:forEach var="d" items="${list}" varStatus="status">
									...
									<td><span data-toggle="tooltip"   title="${(page.page-1)*page.pageSize+status.index + 1}">${(page.page-1)*page.pageSize+status.index + 1}</span></td>
   									 ...
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.cityName }">${d.cityName }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.proId }">${d.proId }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.citySort }">${d.citySort }</span></td>
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/my/City/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>my/city/query.html?pageNo={{number}}',
