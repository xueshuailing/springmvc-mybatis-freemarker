...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>my/tbgoodspic/query.html">
			...
			<table id="myTable" class="table table-condensed table-bordered" >
				<thead>
					...
					<td >序号</td>
   					 ...
   					 <td ></td>
   					 <td ></td>
   					 <td >0 不显示 1 显示 同一件商品只能有一张图片是列表展示的</td>
					</tr>
				</thead>
					<tbody id="tb_tbody">
					<c:choose>
						<c:when  test="${(list)!= null && fn:length(list) > 0}">
							 <c:forEach var="d" items="${list}" varStatus="status">
									...
									<td><span data-toggle="tooltip"   title="${(page.page-1)*page.pageSize+status.index + 1}">${(page.page-1)*page.pageSize+status.index + 1}</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsOriginalPicUrl }">${d.goodsOriginalPicUrl }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsThumbPicUrl }">${d.goodsThumbPicUrl }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsSort }">${d.goodsSort }</span></td>
   									 <td >
   									 ...
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.isShow }">${d.isShow }</span></td>
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/my/TbGoodsPic/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>my/tbgoodspic/query.html?pageNo={{number}}',
