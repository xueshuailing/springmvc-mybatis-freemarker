...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>my/tbfinancial/query.html">
    	...
    	<div class="table-responsive">
			<table id="myTable" class="table table-condensed table-bordered" >
				<thead>
					...
					<td >序号</td>
   					 <td >财务的类型（存储的是财务类型表的id）</td>
   					 <td >金额</td>
   					 <td ></td>
   					 <td >该条财务是由哪个家庭成员产生的 存储的是家庭成员的id</td>
   					 <td >用户id</td>
   					 <td >成员性别 0--男 1--女 主要用来性别统计</td>
   					 <td ></td>
   					 <td >是否收入 0--是 1--不是</td>
   					 <td ></td>
					</tr>
						...
						<c:when  test="${(list)!= null && fn:length(list) > 0}">
							 <c:forEach var="d" items="${list}" varStatus="status">
									...
									<td><span data-toggle="tooltip"   title="${(page.page-1)*page.pageSize+status.index + 1}">${(page.page-1)*page.pageSize+status.index + 1}</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.typeId }">${d.typeId }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.num }">${d.num }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.createDate }">${d.createDate }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.familyId }">${d.familyId }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.userId }">${d.userId }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.familySex }">${d.familySex }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.familyAge }">${d.familyAge }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.flag }">${d.flag }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.remark }">${d.remark }</span></td>
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/my/TbFinancial/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>my/tbfinancial/query.html?pageNo={{number}}',
