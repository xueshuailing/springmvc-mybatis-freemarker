...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>my/tbshopping/query.html">
						...
						<c:when  test="${(list)!= null && fn:length(list) > 0}">
							 <c:forEach var="d" items="${list}" varStatus="status">
									...
									<td><span data-toggle="tooltip"   title="${(page.page-1)*page.pageSize+status.index + 1}">${(page.page-1)*page.pageSize+status.index + 1}</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsId }">${d.goodsId }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsName }">${d.goodsName }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsNum }">${d.goodsNum }</span></td>
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.goodsUrl }">${d.goodsUrl }</span></td>
   									 <td >
   									 ...
   									 <td >
   									 <span  data-toggle="tooltip"   title="${d.shoppingCount }">${d.shoppingCount }</span></td>
   									 <td >
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/my/TbShopping/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>my/tbshopping/query.html?pageNo={{number}}',
