...
<html  lang="zh-CN">
  ...
  <body>
  <form class="form-horizontal" role="form" id="serch" method="post" action="<%=basePath%>${interceptUrl}/${tableInfo.getTableClassName()?lower_case}/query.html">
					...
					<c:choose>
						<c:when  test="${"${"}(list)!= null && fn:length(list) > 0${"}"}">
							 <c:forEach var="d" items="${"${"}list${"}"}" varStatus="status">
									...
									<#if columnInfo.javaName != "id">
   									 <td >
   									 <span  data-toggle="tooltip"   title="${"${"}${"d."}${columnInfo.javaName} ${"}"}">${"${"}${"d."}${columnInfo.javaName} ${"}"}</span></td>
									</#if>
									</#list>
...
<!-- 添加弹出窗口 -->
<jsp:include  page="/WEB-INF/${jsp}/${tableInfo.getTableClassName()}/add.jsp" flush="true"/> 
		<!-- 日期插件js -->
...
$(function () {
  ...
  $('[data-toggle="tooltip"]').tooltip();
        ...
        visiblePages: 7,
         href: '<%=basePath%>${interceptUrl}/${tableInfo.getTableClassName()?lower_case}/query.html?pageNo={{number}}',
