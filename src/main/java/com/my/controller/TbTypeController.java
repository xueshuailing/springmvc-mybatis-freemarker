...
import com.my.service.TbTypeService;
import com.my.entity.TbType;
/**
 * 收入、支出类型字典表controller层
 */
@Controller
@RequestMapping(value = "/my/tbtype")
public class TbTypeController extends BaseController {
	private static final Logger logger = LoggerFactory.getLogger(TbTypeController.class);
	@Resource
	...
	*/
	...
	public String  save(Model model,TbType t,HttpServletRequest request,HttpServletResponse response){
		try {
			 this.tbTypeService.save(t);
		} catch (Exception e) {
	...
	*/
	...
	public String  deleteByParameter(Model model,TbType t,HttpServletRequest request,HttpServletResponse response){
		try {
			 this.tbTypeService.deleteByParameter(t);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	*/
	...
	public String  deleteById(Model model,@RequestParam(value="id", required=true, defaultValue="0") String id,HttpServletRequest request,HttpServletResponse response){
		try {
			 this.tbTypeService.deleteById(id);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	*/
	...
	TbType t=new TbType();
		try {
			  t= this.tbTypeService.detail(id);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
